...
//   指明id是标识字段。再删除更新的时候有用，如果配置此字段在翻页的时候在翻页时候被选中的记录是不会丢失的
 ...
 toolbar: [{
		...
		width:400,
		height:300,
        content:'<iframe src="send_discount_save.action" frameborder="0" width="100%" height="100%" />'
         });  
	  ...
	  $.messager.show({    
	   ...
	   //1.完成弹出更新的页面
		...
		width:320,
		height:250,
        content:'<iframe src="send_discount_update.action" frameborder="0" width="100%" height="100%" />'
         });  
	...
	if (r){
	 ...
	 $.post("discount_deleteByIds.action",
	 {
	 ids:ids
	 },
	 function(result){
    ...
    columns:[[  
        ...
        return "<span title="+value+">"+value+"折"+"</span>"; 
        }
      
        },
         
        {field:'personprice',title:'单价',width:100,
        //用来格式化当前列的值，返回的是最终的数据
        formatter:function(value,row,index){
        return "<span title="+value+">"+value+"元"+"</span>";
        }
        },    
       
        {field:'size.id',title:'企业规模',width:100,
        formatter:function(value,row,index){
            
             var strs= new Array(); //定义一数组 
              strs=row.size.size.split("~"); //字符分割 
             return "<span title="+value+">"+strs[0]+"至"+strs[1]+"人</span>";
